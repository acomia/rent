...
- **Customers may not leave Messenger** — _accepted, deliberately._ The funnel starts on Facebook today, and the app has no messaging to absorb steps 1–3 of the observed workflow. **That is the point:** replacing the need to hunt through Facebook is why this app exists, so the app makes those steps unnecessary rather than supporting them. The bet is that a browsable catalog with real sizes and a size guide answers the question the customer would otherwise have to ask. It follows that anything weakening that answer — thin sizes, missing photos, a calendar that offers dates the shop cannot honour — is not a polish item but a threat to the product's core premise. In-app messaging stays out of v1 and v2 unless the beta shows the catalog genuinely cannot carry the load.
